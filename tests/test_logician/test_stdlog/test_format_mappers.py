...
class TestStdStrFmtMprComputer:
    ...
    @pytest.mark.parametrize("same_fmt_per_lvl", [None, True, False, "%(levelname)s -- %(message)s",
    ...
    class TestStreamSetIsNone:
        ...
        @pytest.mark.parametrize("same_fmt_per_lvl", [None, False,
                                                      "", 0
                                                      ])
        ...
        @pytest.mark.parametrize("same_fmt_per_lvl", [True, "%(name)s",
                                                      1, [1] # not valid values, just for testing purpose
                                                      ])
    ...
    @pytest.mark.parametrize("stream_set, same_fmt_per_level, num_entries, fmt_per_lvl", [
    ...
    class TestMultiplePermutations:
        ...
        def test_only_supplied_streams_included(self, stream_set, same_fmt_per_level, num_entries, fmt_per_lvl, sut):
            """
        ...
        def test_format_mappers_are_of_correct_type(self, stream_set, same_fmt_per_level, num_entries, fmt_per_lvl, sut):
            """
            :param stream_set: the set of streams supplied.
            :param same_fmt_per_level: same format per level?
            :param num_entries: number of entries in the returned dict to verify for assertion.
    ...
    @pytest.mark.parametrize("fmt_per_lvl, stream_set, num_entries", [
        ("%(name)s", {sys.stderr}, 1),
        ("%(name)s -- %(message)s", {sys.stderr, sys.stdout}, 2),
        ("%(name)s -- %(levelname)s -- %(message)s", {sys.stderr, sys.stdout, TextIO()}, 3)
    ])
    class TestSameFmtPerLevelProvidesFmt:
        ...
        def test_only_provided_streams_exist(self, fmt_per_lvl, stream_set, num_entries, sut):
            ret_dict = sut.compute(fmt_per_lvl, stream_set)
            assert len(ret_dict) == num_entries
            assert all(stream in ret_dict for stream in stream_set)

        def test_format_mappers_are_of_correct_type(self, fmt_per_lvl, stream_set, num_entries, sut):
            """
            :param stream_set: the set of streams supplied.
            ...
            """
            ret_dict = sut.compute(fmt_per_lvl, stream_set)
            assert all(isinstance(fpl, StdLogAllLevelSameFmt) for stream, fpl in ret_dict.items())

        @pytest.mark.parametrize("level", [logging.DEBUG, logging.FATAL])
        def test_format_mappers_have_the_supplied_fmt(self, fmt_per_lvl, stream_set, num_entries, level, sut):
            """
            :param stream_set: the set of streams supplied.
            :param fmt_per_lvl: same format per level?
            :param num_entries: number of entries in the returned dict to verify for assertion.
            ...
            """
            ret_dict = sut.compute(fmt_per_lvl, stream_set)
            assert all(fpl.fmt(level)==fmt_per_lvl for stream, fpl in ret_dict.items())
