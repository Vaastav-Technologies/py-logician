...
def test_registers_supplied_unknown_levels():
    ...
    """
    levels = {7: "SEVEN-LVL", 25: "TWENTY-FIVE-LVL"}
...
def test_overrides_supplied_known_levels():
    ...
    """
    levels = {10: "DEBUG-LVL", 20: "INFO-LVL"}
...
def test_supplied_levels_are_not_altered():
    levels = {5: "TRACE-LVL", 10: "DEBUG-LVL", 20: "INFO-LVL"}
...
def test_registers_default_if_not_provided(level_name_map):
    ...
    for level in DirectStdAllLevelLogger.DEFAULT_LEVEL_MAP:
        ...
        assert (
            DirectStdAllLevelLogger.DEFAULT_LEVEL_MAP[level] == registered_levels[level]
        )
...
class TestSimpleWarningVQLevelOrDefault:
    def test_warns_user_by_default(self):
        ...
        with pytest.warns(
            UserWarning,
            match="'vvv': Unexpected verbosity value. Choose from 'v' and 'vv'.",
        ):
    ...
    def test_can_have_a_custom_key_error_handler(self):
        ...
        s = SimpleWarningVQLevelOrDefault[int](
            {"v": 10, "vv": 20}, key_error_handler=_ErrOnly()
        )
        with pytest.raises(
            KeyError,
            match="'vvv': Unexpected verbosity value. Choose from 'v' and 'vv'.",
